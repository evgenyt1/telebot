...
Минимальный бот для **личных переписок Telegram**, подключаемый к аккаунту через Telegram Business Bots. Если собеседник пишет отдельное слово «время» (например, «которое время?»), бот отвечает текущим временем от имени подключённого аккаунта. Упоминать бота в переписке не нужно. Бот обрабатывает только новые входящие текстовые сообщения в выбранных личных чатах; свои сообщения и сообщения других ботов пропускает.
...
После этого попросите другого человека написать «время» в одной из разрешённых личных переписок. Бот ответит временем в часовом поясе `TIME_ZONE` из `.env`. По умолчанию это `Europe/Istanbul`.
